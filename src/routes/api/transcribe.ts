  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }: { request: Request }) => {
        ...
        if (audio.size === 0) return jsonResponse({ ok: false, error: "Áudio vazio." }, 400, request);
        if (audio.size > MAX_BYTES) return jsonResponse({ ok: false, error: "Áudio muito grande (máx 25MB)." }, 413, request);
        ...
        try {
          ...
          if (!r.ok) {
            ...
            return jsonResponse({ ok: false, error: "Falha ao transcrever o áudio." }, 502, request);
          }
