...
// base64 padrão (com ou sem padding). Não valida o conteúdo decodificado,
// mas barra strings claramente não-base64 antes de gastar tokens de visão.
    ...
    handlers: {
      ...
      POST: async ({ request }: { request: Request }) => {
        ...
        try {
          ...
          return jsonResponse({ ok: false, error: "Dados inválidos." }, 400, request);
        }
        const apiKey = process.env.OPENAI_API_KEY;
        if (!apiKey)
          return jsonResponse(
            { ok: false, error: "API key não configurada no servidor." },
            500,
            request,
          );
        if (body.imageBase64.length > 7_000_000)
          return jsonResponse(
            { ok: false, error: "Imagem muito grande (máx ~5 MB)." },
            413,
            request,
          );
        if (!BASE64_RE.test(body.imageBase64))
          return jsonResponse({ ok: false, error: "Imagem inválida." }, 400, request);
        ...
        // Controle de custo: limita análises de imagem por usuário.
        if (!(await enforceAiRateLimit(userId, "ai-scan", { maxPerMin: 10, maxPerDay: 200 }))) {
          return jsonResponse(
            { ok: false, error: "Você atingiu o limite de análises. Aguarde um momento." },
            429,
            request,
          );
        }
        ...
        try {
          ...
          if (!r.ok) {
            ...
            if (r.status === 429)
              return jsonResponse(
                { ok: false, error: "Limite de requisições. Aguarde um momento." },
                429,
                request,
              );
            if (r.status === 401)
              return jsonResponse({ ok: false, error: "API key inválida." }, 500, request);
          ...
          if (!parsed)
            return jsonResponse(
              {
                ok: false,
                error: "Não foi possível interpretar a resposta. Tente uma imagem mais nítida.",
              },
              502,
              request,
            );
