...
import { Lock, LogOut, Search, ShieldCheck, Crown, RefreshCw, Users, Copy, Check } from "lucide-react";
...
function AdminPage() {
  ...
  return (
    <div className="min-h-dvh" style={{ background: "var(--background)", color: "var(--foreground)" }}>
      {authed ? <Panel onLogout={() => setAuthed(false)} /> : <LoginCard onAuthed={() => setAuthed(true)} />}
    </div>
  );
}
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center px-4">
      <form
        ...
        style={{ background: "var(--surface)", borderColor: "var(--border-strong)", boxShadow: "0 30px 80px -40px color-mix(in oklab, var(--accent) 50%, transparent)" }}
      >
        ...
        <label className="mt-6 block text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Senha de acesso</label>
        <div className="mt-2 flex items-center gap-2 rounded-xl border px-3 smooth focus-within:border-[color:var(--accent)]"
...
function Panel({ onLogout }: { onLogout: () => void }) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (!ql) return true;
      return (r.email || "").toLowerCase().includes(ql) || (r.name || "").toLowerCase().includes(ql);
    });
  }, [rows, filter, q]);
  ...
  async function doPromote(id: string) {
    ...
    try {
      ...
      setRows((rs) => rs.map((r) => r.user_id === id ? { ...r, is_pro: true, analyses_left: 9999, trial_days_left: 365 } : r));
      toast.success("Promovido para PRO Anual.");
    } catch { toast.error("Falha ao promover."); }
  ...
  async function doDemote(id: string) {
    ...
    try {
      ...
      setRows((rs) => rs.map((r) => r.user_id === id ? { ...r, is_pro: false, analyses_left: 5, trial_days_left: 7 } : r));
      toast.success("Acesso revertido para Free.");
    } catch { toast.error("Falha ao despromover."); }
    finally { setBusy(null); }
  }
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-5 py-8">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="text-[10px] font-semibold uppercase tracking-[0.3em] text-muted-foreground">OrionHub</div>
          <h1 className="font-display text-2xl font-extrabold tracking-tight">Painel Admin</h1>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={refresh} className="inline-flex items-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-medium smooth press hover:border-[color:var(--accent)] hover:text-[color:var(--accent)]"
            style={{ borderColor: "var(--border-strong)", background: "var(--surface)" }}>
            <RefreshCw className={`h-3.5 w-3.5 ${loading ? "animate-spin" : ""}`} /> Atualizar
          </button>
          <button onClick={doLogout} className="inline-flex items-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-medium smooth press hover:border-[color:var(--red)] hover:text-[color:var(--red)]"
            style={{ borderColor: "var(--border-strong)", background: "var(--surface)" }}>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <div className="flex items-center gap-1 rounded-lg border p-1" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
          {(["all","free","pro"] as const).map((k) => (
            <button key={k} onClick={() => setFilter(k)}
              className="rounded-md px-3 py-1.5 text-xs font-semibold smooth"
              style={filter === k
                ? { background: "color-mix(in oklab, var(--accent) 18%, var(--surface-2))", color: "var(--foreground)" }
                : { color: "var(--text-muted)" }}>
              {k === "all" ? "Todos" : k === "free" ? "Free" : "PRO Anual"}
            </button>
          ))}
        </div>
        <div className="flex flex-1 min-w-[220px] items-center gap-2 rounded-lg border px-3"
        ...
        {!loading && filtered.map((r) => (
          <div key={r.user_id} className="grid grid-cols-12 items-center gap-3 border-b px-4 py-3 text-sm smooth hover:bg-[color:var(--surface-2)]" style={{ borderColor: "var(--border)" }}>
            <div className="col-span-5 min-w-0">
              ...
              <button onClick={() => copyEmail(r.email)} className="mt-0.5 inline-flex items-center gap-1.5 truncate text-[11px] text-muted-foreground hover:text-[color:var(--accent)]">
                {copied === r.email ? <Check className="h-3 w-3" /> : <Copy className="h-3 w-3" />}
            ...
            <div className="col-span-2">
              {r.is_pro ? (
                ...
                <span className="inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-muted-foreground"
            ...
            <div className="col-span-3 flex justify-end gap-2">
              {r.is_pro ? (
                ...
                <button disabled={busy === r.user_id} onClick={() => doPromote(r.user_id)}
...
function Stat({ label, value, icon, accent }: { label: string; value: number; icon: React.ReactNode; accent?: boolean }) {
  return (
    <div className="rounded-2xl border p-4 card-glow" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
      <div className="flex items-center justify-between text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
        <span>{label}</span>
        <span style={{ color: accent ? "var(--accent)" : "var(--text-dim)" }}>{icon}</span>
      </div>
      <div className="mt-1 font-display text-3xl font-black tabular" style={{ color: accent ? "var(--accent)" : "var(--foreground)" }}>{value}</div>
    </div>
  );
}