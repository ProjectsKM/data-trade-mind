...
function Countdown() {
  ...
  const cell = (n: number, label: string) => (
    <div className="rounded-xl border px-3 py-2 text-center tabular" style={{ background: "var(--surface)", borderColor: "var(--border-strong)" }}>
  ...
  return (
    <div className="flex items-center gap-2">
      {cell(d, "dias")}{cell(h, "h")}{cell(m, "min")}{cell(s, "seg")}
    </div>
  );
}
  ...
  return (
    <section className="relative z-10 px-6 pt-10 pb-14 md:px-12 md:pt-16 md:pb-20">
      <div className="mx-auto max-w-5xl text-center">
        <div className="mb-5 inline-flex items-center gap-2 rounded-full border px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest fade-down"
        ...
        <h1 className="font-display text-4xl font-black leading-[1.05] tracking-tight md:text-6xl fade-up">
          A plataforma de <span className="gradient-text">trading inteligente</span><br className="hidden md:block" />
    ...
    <section className="relative z-10 px-6 py-16 md:px-12">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-8 overflow-hidden rounded-3xl border" style={{ background: "var(--surface)", borderColor: "var(--border-strong)" }}>
          ...
          {COMPARE_ROWS.map((r) => (
            <div key={r.label} className="grid grid-cols-12 items-center border-b px-5 py-3.5 smooth hover:bg-[color:var(--surface-2)]" style={{ borderColor: "var(--border)" }}>
              <div className="col-span-6 text-sm">{r.label}</div>
              <div className="col-span-3 flex justify-center"><Cell v={r.free} /></div>
              <div className="col-span-3 flex justify-center"><Cell v={r.pro} /></div>
            </div>
          ))}
        </div>
      </div>
...
const MODULES = [
  ...
  { Icon: Calculator, name: "Calculadora de Banca", desc: "Gerencie risco por operação, payout, sequência e meta diária." },
  { Icon: ClipboardList, name: "Gestão de Trades", desc: "Planilha completa com métricas de win rate, profit factor e drawdown." },
];
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 stagger">
          {MODULES.map(({ Icon, name, desc }) => (
            <div key={name} className="group rounded-2xl border p-5 card-glow hover-lift" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
  ...
  return (
    <section className="relative z-10 px-6 py-16 md:px-12">
      <div className="mx-auto max-w-2xl">
        <div className="rounded-3xl border p-8 text-center card-glow"
          style={{
...
function Mentor() {
  return (
    <section className="relative z-10 px-6 py-16 md:px-12">
      <div className="mx-auto grid max-w-5xl items-center gap-8 rounded-3xl border p-8 md:grid-cols-[1fr_2fr]"
        style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
        <div className="flex items-center justify-center">
          <div className="flex h-32 w-32 items-center justify-center rounded-full ring-gradient" style={{ background: "var(--surface-2)" }}>
            <span className="font-display text-4xl font-black gradient-text">GD</span>
          </div>
        </div>
        <div>
          <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">O Mentor</div>
          <h3 className="mt-1 font-display text-2xl font-extrabold tracking-tight">Gabriel Dutra</h3>
          <p className="mt-1 text-sm" style={{ color: "var(--accent)" }}>Trader e professor do grupo Orion Capital</p>
          <p className="mt-3 text-sm text-muted-foreground">
            Após anos formando traders consistentes, Gabriel reuniu sua metodologia em uma plataforma única.
            O OrionHub é a tradução prática do que ele ensina dentro do Orion Capital — agora com IA para
            acelerar suas leituras e gestão.
          </p>
        </div>
      </div>
    </section>
  ...
  return (
    <section className="relative z-10 px-6 py-16 md:px-12">
      <div className="mx-auto max-w-3xl">
        <h2 className="text-center font-display text-3xl font-black tracking-tight">Perguntas frequentes</h2>
        <div className="mt-8 space-y-2">
          {FAQS.map(([q, a], i) => (
            <div key={q} className="overflow-hidden rounded-2xl border smooth" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
              <button onClick={() => setOpen(open === i ? null : i)} className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left text-sm font-semibold smooth hover:text-[color:var(--accent)]">
                <span>{q}</span>
                <span className="text-xl text-muted-foreground">{open === i ? "−" : "+"}</span>
              </button>
              {open === i && (
...
function FinalCta() {
  return (
    <section className="relative z-10 px-6 py-20 md:px-12">
      <div className="mx-auto max-w-3xl rounded-3xl border p-10 text-center card-glow"
        style={{
          background: "linear-gradient(135deg, color-mix(in oklab, var(--accent) 18%, var(--surface)), color-mix(in oklab, var(--electric) 10%, var(--surface)))",
          borderColor: "color-mix(in oklab, var(--accent) 40%, transparent)",
        }}>
        <Zap className="mx-auto h-8 w-8" style={{ color: "var(--electric)" }} />
        <h3 className="mt-3 font-display text-3xl font-black tracking-tight">Pronto para entrar no OrionHub?</h3>
...
function Footer() {
  return (
    <footer className="relative z-10 border-t px-6 py-8 text-center text-xs text-muted-foreground" style={{ borderColor: "var(--border)" }}>
      © {new Date().getFullYear()} OrionHub · Todos os direitos reservados.
      <span className="mx-2">·</span>
      <Link to="/" className="hover:text-[color:var(--accent)]">Voltar ao site</Link>
    </footer>
  );
}

// silence unused
void InfinityIcon; void Gem;